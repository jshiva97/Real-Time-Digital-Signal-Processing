...
float *m1,*m2,*m3,*m4,*mag,*noisemag,*temp,*g,*p;
float ALPHA=20;
float LAMBDA=0.1;
float TAU = 0.02;
 /******************************* Function prototypes *******************************/
...
void process_frame(void)
{
	...
	for(i=0;i<FFTLEN;i++)
	{
		...
		switch(enhance)
		{
			case 1: p[i] = (1-k)*mag[i] + k*p[i-1];
					mag[i] = p[i];
					break;
			case 2: p[i] = (1-k)*mag[i]*mag[i] + k*p[i-1];
					mag[i] = sqrt(p[i]);
					break;
			case 3: p[i] = (1-k)*mag[i]*mag[i] + k*p[i-1];
					mag[i] = sqrt(p[i]);
					break;
			default:mag[i] = mag[i];		
		}
		...
		switch(enhance)
		{
			case 3: noisemag[i] = (1-k)*noisemag[i] + k*noisemag[i-1];
					break;
