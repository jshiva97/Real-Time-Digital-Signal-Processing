...
volatile int enhancement = 1;
volatile int counter = 0;
float *m1,*m2,*m3,*m4,*mag,*noisemag,*temp,*p,*noisemag_prev,*ratio_prev;
complex *out_prev, *out_cur, *out_next;
float ALPHA=3;
float LAMBDA=0.01;
float TAU = 0.08;
float lpf_const,threshold=10,gain=10;
...
void enhance()
{
	...
	for(i=0;i<FFTLEN;i++)
	{
		...
		g = max(LAMBDA,1-alpha_cur*noisemag[i]/p[i]);	
		out_next[i].r = g*inframe[i].r;
		out_next[i].i = g*inframe[i].i;
