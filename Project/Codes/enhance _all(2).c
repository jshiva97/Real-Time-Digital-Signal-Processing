...
volatile int enhance = 10;
volatile int option = 5;
volatile int counter = 0;
float *m1,*m2,*m3,*m4,*mag,*noisemag,*temp,*g,*p,*p_prev,*p2,*p2_prev,*noisemag_prev,*ratio_prev;
complex *out_prev, *out_cur, *out_next;
float ALPHA=1;
float LAMBDA=0.005;
float TAU = 0.08;
...
/*  Initialize and zero fill arrays */  
	...
	out_prev 	= (complex *) calloc(FFTLEN, sizeof(complex));
	out_cur		= (complex *) calloc(FFTLEN, sizeof(complex));
	out_next	= (complex *) calloc(FFTLEN, sizeof(complex));
	ratio_prev	= (float *) calloc(FFTLEN, sizeof(float));
...
void enhance_7()
{
	...
	if(counter==0)
	{
		...
		SNR = 20*log(p[i]/noisemag[i]);
		if((SNR<10)&&(SNR>5))
			alpha_cur = ALPHA*gain*0.5;
		else if((SNR<5)&&(SNR>0))
			alpha_cur = ALPHA*gain;
		else if((SNR<0)&&(SNR>-5))
			alpha_cur = ALPHA*gain*gain;
		else if(SNR<-5)
			alpha_cur = ALPHA*gain*gain*gain;
		else
			alpha_cur = ALPHA;
	}
	for(i=0;i<FFTLEN;i++)
	{
		...
		p[i] = (1-lpf_const)*mag[i] + lpf_const*p[i];
		mag[i] = p[i];
		if(m1[i]>mag[i])
			m1[i] = mag[i];
		noiseratio = noisemag[i]/mag[i];
		noiseratio = noiseratio*alpha_cur;
		switch(option)
		{
//			case 1: g[i] = max(LAMBDA,1-noiseratio);
//					break;
//			case 2: g[i] = max(LAMBDA*noiseratio,1-noiseratio);
//					break;
			case 3: g[i] = max(LAMBDA*p[i]/mag[i],1-noiseratio);
					break;
//			case 4: g[i] = max(LAMBDA*noisemag[i]/p[i],1-noisemag[i]/p[i]);
//					break;
			case 5: g[i] = max(LAMBDA,1-alpha_cur*noisemag[i]/p[i]);
					break;
//			case 6: g[i] = max(LAMBDA,sqrt(1-noiseratio*noiseratio));
//					break;
			default:g[i] = max(LAMBDA,1-noiseratio);			
...
//			noisemag[i] = rejection*noisemag[i];
			noisemag[i] = (1-lpf_const)*noisemag[i]+lpf_const*noisemag_prev[i];
//			noisemag[i] = (1-hpf_const)*noisemag[i]-hpf_const*noisemag_prev[i];
			noisemag[FFTLEN-i] = noisemag[i];
		}
		SNR = 20*log(mag[i]/noisemag[i]);
		if((SNR<upper_bound)&&(SNR>0))
			alpha_cur = ALPHA*gain;
		else if((SNR<0)&&(SNR>-5))
			alpha_cur = ALPHA*gain*gain;
		else if(SNR<-5)
...
//			alpha_cur = ALPHA*gain*gain*gain;
	...
	for(i=0;i<FFTLEN;i++)
	{
		...
		p[i] = (1-lpf_const)*mag[i]*mag[i] + lpf_const*p[i];
		mag[i] = sqrt(p[i]);
		if(m1[i]>mag[i])
			m1[i] = mag[i];
		noiseratio = alpha_cur*noisemag[i]/mag[i];
		switch(option)
		{
//			case 1: g[i] = max(LAMBDA,1-noiseratio);
//					break;
...
//					break;
		...
		out_next[i].r = g[i]*inframe[i].r;
		out_next[i].i = g[i]*inframe[i].i;
		outframe[i]=out_cur[i];
		if(ratio_prev[i]>=threshold)
		{
			if(cabs(outframe[i])>cabs(out_prev[i]))
			{
				outframe[i] = out_prev[i];
			}
			if(cabs(outframe[i])>cabs(out_next[i]))
			{
				outframe[i] = out_next[i];	
			}
		}
		ratio_prev[i] = noiseratio;
	}				      	
	ifft(FFTLEN,outframe);	
}
...
//		SNR = 20*log(mag[i]/noisemag[i]);
//		if((SNR<upper_bound)&&(SNR>-5))
//			alpha_cur = ALPHA - SNR/s;
////		else if((SNR<0)&&(SNR>-5))
////			alpha_cur = ALPHA*gain*gain;
//		else if(SNR<-5)
//			alpha_cur = ALPHA + 5/s;
//		else
//			alpha_cur = ALPHA + 20/s;
	}
...
//			case 4: g[i] = max(LAMBDA*noisemag[i]/p2[i],1-noisemag[i]/p2[i]);
//					break;
			case 5: g[i] = max(LAMBDA,1-alpha_cur*noisemag[i]/p2[i]);
					break;
//			case 6: g[i] = max(LAMBDA,sqrt(1-noiseratio*noiseratio));
//					break;
			default:g[i] = max(LAMBDA,1-noiseratio);			
...
void enhance_10()
{
	...
	for(i=0;i<FFTLEN;i++)
	{
		...
		g[i] = max(LAMBDA*p2[i]/mag[i],1-noisemag[i]/mag[i]);
		outframe[i].r = g[i]*inframe[i].r;
		outframe[i].i = g[i]*inframe[i].i;
	}				      	
	ifft(FFTLEN,outframe);	
