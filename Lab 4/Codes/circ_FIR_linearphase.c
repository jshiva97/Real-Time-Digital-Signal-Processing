...
void init_HWI(void);                  
void ISR_AIC(void);	
void circ_FIR();				//function that implements the FIR
/********************************** Main routine ************************************/
void main(){      
...
void circ_FIR()							//circular buffer implementation of FIR with symmetry
{
	int i,j,k;
	...
	{
		if(i==-1)						//conditional statement to check wrap around of i
		{
			i = N;
		}
		if(j==N+1)						//conditional statement to check wrap around of j
		{
			j = 0;
		}
		output += b[k]*(x[i--]+x[j++]);
	...
	if(j==N+1)							//conditional statement to check wrap around of j
	{									//before middle element is processed
		j=0;
	}
