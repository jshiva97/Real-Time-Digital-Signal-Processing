...
void ISR_AIC()
{
	...
	sample = (Int16) (sample * L_Gain);
	mono_write_16Bit(abs(sample));
}
...
float sinegen(void)
{
	...
	index += (2*SINE_TABLE_SIZE*sine_freq/sampling_freq); //offset definition
	while(index>=SINE_TABLE_SIZE)
	{
		//while index exceeds the table size, table size is subtracted from index
		index-=SINE_TABLE_SIZE ;
	}	
	
	//set the output as the value of the sine wave, stored in table
	wave = table[(int)floor(index)];
		   
    return(wave); 
    
}

//fills table with values of 256 equally spaced points around half a sine wave
void sine_init()
{
	int x;
	for(x=0; x<SINE_TABLE_SIZE; x++){
		table[x] = sin((PI*x)/SINE_TABLE_SIZE);	
	}
}
