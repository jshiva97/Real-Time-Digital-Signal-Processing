...
//Table containing values of sine wave
...
/*  This code produces a fixed sine of 1KHZ (if the sampling frequency is 8KHZ)
	...
	index += (SINE_TABLE_SIZE*sine_freq/sampling_freq); //offset definition
	if(index>=SINE_TABLE_SIZE)
	{
		//when index exceeds the table size, table size is subtracted from index
...
//fills table with values of 256 equally spaced points around the sine wave
void sine_init()
{
	int x;
	for(x=0; x<SINE_TABLE_SIZE; x++){
		table[x] = sin((2*PI*x)/SINE_TABLE_SIZE);	
	}
}
